...
def assign_space(total_size, specs):
    ...
    ret = []
    for s in specs:
        num_blocks = int((float(s.weight) / total_weight) * (total_size / float(s.size)))
        s.set_count(num_blocks)
        total_size -= num_blocks * s.size
...
def partition(diskname, specs, force=False, check_mode=False):
    ...
    for spec in specs:
        for i in range(spec.count):
            ...
            partition = parted.Partition(disk=disk, type=parted.PARTITION_NORMAL, fs=fs, geometry=geo)
...
def get_partitions(diskpath):
    ...
    for p in disk.partitions:
        (_, _, pname) = p.path.rsplit("/")
        partitions.append({"name": pname, "size": p.getLength() * dev.sectorSize})
...
def main():
    ...
    try:
        ...
        err = "Error parsing sizes=" + sizes + ": " + str(e)
    ...
    for disk in disks.split(","):
        try:
            ...
            err = "Error creating partitions on " + disk + ": " + str(e)
            raise
            #module.fail_json(msg=err)
